...
abstract class CategoryCommand extends Command {
  ...
  async create(message: Message) {
    ...
    const newChannel = await message.guild?.channels.create(categoryName, { type: "category", permissionOverwrites: [{ id: message.guild?.id, deny: ['VIEW_CHANNEL'] }] });
    const newCategory = { id: Date.now() + (message.guild?.id || ''), serverID: message.guild?.id || "", channelID: newChannel?.id || "", name: categoryName, userID: (message.member?.id || '') };
  ...
  async move(message: Message) {
    ...
    const voiceChannelData = await this.client.db.ChannelsByMeModel.findOne({ where: { serverID: (message.guild?.id || ""), channelID: message.member.voice?.channelID, userID: message.member.id } });
    if(!voiceChannelData) {
